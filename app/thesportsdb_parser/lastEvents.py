...
from thesportsdb.events import lastLeagueEvents
import logging
...
async def get_last_events_api(pool: asyncpg.pool.Pool) -> list:
...
async def work_with_last_events(pool: asyncpg.pool.Pool):
    events = await get_last_events_api(pool)
